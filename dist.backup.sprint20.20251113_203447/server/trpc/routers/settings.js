...
import { systemSettings, notificationPreferences, securitySettings, systemBackups, aiProviders, externalServices, } from '../../db/schema.js';
...
export const settingsRouter = router({
    ...
    list: publicProcedure
        ...
        const userSettings = await db
            .select()
            .from(systemSettings)
            .where(and(eq(systemSettings.userId, userId), conditions.length > 0 ? and(...conditions) : undefined))
            .limit(input.limit)
            .offset(input.offset);
    ...
    getByKey: publicProcedure
        ...
        const setting = await db
            .select()
            .from(systemSettings)
            .where(and(eq(systemSettings.userId, userId), eq(systemSettings.settingKey, input.settingKey)))
            .limit(1);
    ...
    upsertNotification: publicProcedure
        ...
        const existing = await db
            .select()
            .from(notificationPreferences)
            .where(and(eq(notificationPreferences.userId, userId), eq(notificationPreferences.channel, input.channel), eq(notificationPreferences.eventType, input.eventType)))
            .limit(1);
        if (existing.length > 0) {
            await db
    ...
    getStatistics: publicProcedure.query(async ({ ctx }) => {
        ...
        const notificationsEnabled = await db
            .select({ count: count() })
            .from(notificationPreferences)
            .where(and(eq(notificationPreferences.userId, userId), eq(notificationPreferences.isEnabled, true)));
        ...
        const activeIntegrations = await db
            .select({ count: count() })
            .from(externalServices)
            .where(and(eq(externalServices.userId, userId), eq(externalServices.isActive, true)));
        // Último backup
