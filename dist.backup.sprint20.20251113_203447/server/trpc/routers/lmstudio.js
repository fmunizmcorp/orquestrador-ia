...
export const lmstudioRouter = router({
    ...
    generateCompletion: publicProcedure
        ...
        const result = await lmstudioService.generateCompletion(input.modelId, input.prompt, {
    ...
    switchModel: publicProcedure
        ...
        .mutation(async ({ input }) => {
        const result = await lmstudioService.switchModel(input.preferredModelId, input.fallbackModelIds);
    ...
    compareModels: publicProcedure
        ...
        .mutation(async ({ input }) => {
        const results = await lmstudioService.compareModels(input.modelIds, input.prompt, {
            temperature: input.temperature,
            maxTokens: input.maxTokens,
        });
    ...
    processLongText: publicProcedure
        ...
        .mutation(async ({ input }) => {
        const result = await lmstudioService.processLongText(input.modelId, input.text, (chunk, index, total) => `${input.instruction}\n\nChunk ${index}/${total}:\n${chunk}`);
        return { success: true, result };
    }),
});
