...
export const templatesRouter = router({
    ...
    list: publicProcedure
        ...
        if (input.query) {
            conditions.push(or(like(aiTemplates.name, `%${input.query}%`), like(aiTemplates.description, `%${input.query}%`)));
        }
        if (input.category) {
    ...
    create: publicProcedure
        ...
        const result = await db.insert(aiTemplates).values({
            userId: ctx.userId || 1,
            name: input.name,
    ...
    search: publicProcedure
        ...
        .query(async ({ input, ctx }) => {
        const conditions = [
            or(like(aiTemplates.name, `%${input.query}%`), like(aiTemplates.description, `%${input.query}%`)),
        ];
        if (input.isPublic) {
            conditions.push(eq(aiTemplates.isPublic, true));
        }
    ...
    validateVariables: publicProcedure
        ...
        requiredVariables.forEach((varDef) => {
