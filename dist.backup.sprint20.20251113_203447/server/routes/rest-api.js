...
async function recalculateProjectProgress(projectId) {
    try {
        ...
        if (projectTasks.length > 0) {
            ...
            const updateData = { progress: calculatedProgress };
...
router.post('/projects', async (req, res) => {
    try {
        ...
        if (!name)
            return res.status(400).json(errorResponse({ message: 'Name required' }, 400));
...
router.post('/prompts', async (req, res) => {
    try {
        ...
        if (!title || !content)
            return res.status(400).json(errorResponse({ message: 'Title and content required' }, 400));
...
router.post('/tasks', async (req, res) => {
    try {
        ...
        if (!title)
            return res.status(400).json(errorResponse({ message: 'Title required' }, 400));
...
router.put('/projects/:id', async (req, res) => {
    try {
        const id = parseInt(req.params.id);
        if (isNaN(id))
            return res.status(400).json(errorResponse({ message: 'Invalid ID' }, 400));
        ...
        const [project] = await db.select().from(projects).where(eq(projects.id, id)).limit(1);
        if (!project)
            return res.status(404).json(errorResponse({ message: 'Project not found' }, 404));
...
router.put('/teams/:id', async (req, res) => {
    try {
        ...
        const [team] = await db.select().from(teams).where(eq(teams.id, id)).limit(1);
        if (!team)
            return res.status(404).json(errorResponse({ message: 'Team not found' }, 404));
...
router.put('/prompts/:id', async (req, res) => {
    try {
        ...
        const [prompt] = await db.select().from(prompts).where(eq(prompts.id, id)).limit(1);
        if (!prompt)
            return res.status(404).json(errorResponse({ message: 'Prompt not found' }, 404));
...
router.put('/tasks/:id', async (req, res) => {
    try {
        ...
        const [task] = await db.select().from(tasks).where(eq(tasks.id, id)).limit(1);
        if (!task)
            return res.status(404).json(errorResponse({ message: 'Task not found' }, 404));
...
router.post('/models/:id/load', async (req, res) => {
    try {
        ...
        try {
            ...
            const isActuallyLoaded = loadedModels.some((m) => m.id === model.modelId || m.id.includes(model.modelId || ''));
            ...
            if (!isActuallyLoaded) {
                ...
                return res.status(400).json({
                    ...
                    availableModels: loadedModels.map((m) => m.id),
                    simulated: false,
                });
            }
            ...
            const loadResult = {
                ...
                lmStudioModelId: loadedModels.find((m) => m.id === model.modelId || m.id.includes(model.modelId || ''))?.id,
...
router.post('/models/:id/unload', async (req, res) => {
    try {
        const id = parseInt(req.params.id);
        if (isNaN(id)) {
            return res.status(400).json(errorResponse('Invalid model ID'));
        ...
        try {
            ...
            const isStillLoaded = loadedModels.some((m) => m.id === model.modelId || m.id.includes(model.modelId || ''));
    ...
    try {
        ...
        if (role === 'user') {
            try {
                ...
                // Save AI response
                const aiResult = await db.insert(messages).values({
                    conversationId,
                    content: aiContent,
                    role: 'assistant',
                });
...
router.post('/workflows', async (req, res) => {
    try {
        ...
        const result = await db.insert(aiWorkflows).values({
            userId,
            name,
            description,
            steps: steps,
    ...
    try {
        ...
        try {
            ...
            if (isLMStudioAvailable) {
                ...
                let targetModel = loadedModels.find((m) => m.id === model.modelId ||
