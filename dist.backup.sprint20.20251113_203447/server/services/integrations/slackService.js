...
class SlackService {
    async getToken(userId) {
        const [cred] = await db.select().from(credentials)
            .where(and(eq(credentials.userId, userId), eq(credentials.service, 'slack'))).limit(1);
        if (!cred)
    ...
    async postMessage(userId, channel, text, blocks) {
    ...
    async getChannelHistory(userId, channel, limit = 100) {
        const token = await this.getToken(userId);
    ...
    async uploadFile(userId, channels, content, filename, title) {
        const token = await this.getToken(userId);
