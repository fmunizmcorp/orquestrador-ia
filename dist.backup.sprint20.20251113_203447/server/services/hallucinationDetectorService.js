...
class HallucinationDetectorService {
    ...
    async recoverFromHallucination(taskId, subtaskId, savedProgress) {
        try {
            // 1. Buscar subtask
            ...
            if (!alternativeModelId || alternativeModelId === originalModelId) {
                const fallbacks = alternativeAI.fallbackModelIds
                    ? alternativeAI.fallbackModelIds
                    : [];
                alternativeModelId = fallbacks.find((id) => id !== originalModelId) || null;
            }
            ...
            const newResult = await lmstudioService.generateCompletion(alternativeModel.modelId, recoveryPrompt, { temperature: 0.5, maxTokens: 4096, timeout: 300000 });
...
Retorne JSON:
{
  ...
  "reasoning": "explicação da escolha"
            ...
            const comparisonResponse = await lmstudioService.generateCompletion(alternativeModel.modelId, comparisonPrompt, { temperature: 0.3, maxTokens: 3000 });
            let comparison;
            try {
  ...
  "contradictions": [
    {
      ...
      "explanation": "por que contradizem"
            ...
            const response = await lmstudioService.generateCompletion(model.modelId, prompt, { temperature: 0.3, maxTokens: 1024, timeout: 60000 });
            let analysis;
            try {
            ...
            return {
                ...
                examples: (analysis.contradictions || []).map((c) => `"${c.statement1}" contradiz "${c.statement2}"`)
            };
        }
        catch (error) {
    ...
    async isOutOfScope(text, context) {
        try {
...
Retorne JSON:
{
  ...
  "explanation": "breve explicação"
            ...
            const response = await lmstudioService.generateCompletion(model.modelId, prompt, { temperature: 0.3, maxTokens: 512, timeout: 60000 });
            let analysis;
            try {
                analysis = JSON.parse(response);
            }
