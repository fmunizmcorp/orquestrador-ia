...
export const workflowsRouter = router({
    ...
    list: publicProcedure
        ...
        if (input.query) {
            conditions.push(or(like(aiWorkflows.name, `%${input.query}%`), like(aiWorkflows.description, `%${input.query}%`)));
        }
    ...
    search: publicProcedure
        ...
        const conditions = [
            ...
            or(like(aiWorkflows.name, `%${input.query}%`), like(aiWorkflows.description, `%${input.query}%`)),
        ];
        if (input.isActive !== undefined) {
            conditions.push(eq(aiWorkflows.isActive, input.isActive));
        }
    ...
    getStats: publicProcedure
        ...
        const stats = {
            ...
            averageSteps: allWorkflows.length > 0
                ? allWorkflows.reduce((sum, w) => sum + (w.steps?.length || 0), 0) / allWorkflows.length
                : 0,
        };
        return {
            success: true,
