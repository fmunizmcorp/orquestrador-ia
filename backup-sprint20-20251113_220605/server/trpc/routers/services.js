...
export const servicesRouter = router({
    ...
    githubCreateIssue: publicProcedure
        ...
        .mutation(async ({ input }) => {
        const issue = await githubService.createIssue(1, input.owner, input.repo, {
            title: input.title,
            body: input.body,
            labels: input.labels,
        });
    ...
    driveUploadFile: publicProcedure
        ...
        .mutation(async ({ input }) => {
        const file = await driveService.uploadFile(1, input.name, input.content, input.mimeType, input.folderId);
        return { success: true, file };
    }),
    ...
    driveShareFile: publicProcedure
        ...
        .mutation(async ({ input }) => {
        const permission = await driveService.shareFile(1, input.fileId, input.email, input.role);
    ...
    sheetsWriteRange: publicProcedure
        ...
        .mutation(async ({ input }) => {
        const result = await sheetsService.updateSpreadsheetValues(1, input.spreadsheetId, input.range, input.values);
    ...
    sheetsAppendRow: publicProcedure
        ...
        .mutation(async ({ input }) => {
        const result = await sheetsService.appendSpreadsheetValues(1, input.spreadsheetId, input.sheetName, [input.values]);
        return { success: true, result };
    }),
    sheetsCreateSpreadsheet: publicProcedure
        ...
        .mutation(async ({ input }) => {
        const spreadsheet = await sheetsService.createSpreadsheet(1, input.title, input.sheetNames);
        return { success: true, spreadsheet };
    }),
    /**
