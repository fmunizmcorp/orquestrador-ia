...
class SystemMonitorService {
    constructor() {
        Object.defineProperty(this, "DEFAULT_LIMITS", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: {
        ...
        Object.defineProperty(this, "alertIdCounter", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: 0
    ...
    async getMetrics() {
        try {
            const [cpuData, memData, cpuTemp, diskData, networkData, processes, graphics,] = await Promise.all([
                si.currentLoad(),
                si.mem(),
                si.cpuTemperature(),
                si.fsSize(),
            ...
            const gpu = graphics.controllers.map(controller => ({
                ...
                vramFree: (controller.vram || 0) - (parseFloat(controller.vramDynamic) || 0),
                vramUsagePercent: controller.vram
                    ? parseFloat((((parseFloat(controller.vramDynamic) || 0) / controller.vram) * 100).toFixed(2))
                    : 0,
            ...
            const lmstudioProc = processes.list.find(p => p.name.toLowerCase().includes('lmstudio') ||
     ...
     * Verificar e criar alertas
     */
    async checkAndCreateAlerts(metrics) {
     ...
     */
    async checkLimits(customLimits) {
     ...
     */
    async *monitorContinuous(intervalMs = 10000) {
        while (true) {
