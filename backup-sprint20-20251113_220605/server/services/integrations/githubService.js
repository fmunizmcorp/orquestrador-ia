...
class GitHubService {
    ...
    async getCredentials(userId) {
        const [cred] = await db.select()
            .from(credentials)
            .where(and(eq(credentials.userId, userId), eq(credentials.service, 'github')))
            .limit(1);
        if (!cred) {
     ...
     * Listar repositórios
     */
    async listRepos(userId, options) {
     ...
     * Obter repositório
     */
    async getRepo(userId, owner, repo) {
        return this.request(userId, 'GET', `/repos/${owner}/${repo}`);
    }
    /**
     * Criar repositório
     */
    async createRepo(userId, options) {
     ...
     * Deletar repositório
     */
    async deleteRepo(userId, owner, repo) {
    ...
    async createBranch(userId, owner, repo, branchName, fromBranch = 'main') {
        ...
        const fromRef = await this.request(userId, 'GET', `/repos/${owner}/${repo}/git/ref/heads/${fromBranch}`);
        // Create new branch
     ...
     * Listar issues
     */
    async listIssues(userId, owner, repo, options) {
        const params = new URLSearchParams();
        if (options?.state)
     ...
     * Criar issue
     */
    async createIssue(userId, owner, repo, options) {
    ...
    async closeIssue(userId, owner, repo, issueNumber) {
        return this.request(userId, 'PATCH', `/repos/${owner}/${repo}/issues/${issueNumber}`, { state: 'closed' });
    }
    /**
     * Listar Pull Requests
     */
    async listPRs(userId, owner, repo, options) {
        const params = new URLSearchParams();
        if (options?.state)
            params.append('state', options.state);
     ...
     * Criar Pull Request
     */
    async createPR(userId, owner, repo, options) {
     ...
     * Merge Pull Request
     */
    async mergePR(userId, owner, repo, prNumber, commitMessage) {
        return this.request(userId, 'PUT', `/repos/${owner}/${repo}/pulls/${prNumber}/merge`, { commit_message: commitMessage });
    }
    /**
     * Listar commits
     */
    async listCommits(userId, owner, repo, options) {
        const params = new URLSearchParams();
        if (options?.branch)
    ...
    async getFileContent(userId, owner, repo, path, branch) {
        ...
        const response = await this.request(userId, 'GET', `/repos/${owner}/${repo}/contents/${path}${params}`);
        // Decode base64
    ...
    async createOrUpdateFile(userId, owner, repo, path, content, message, branch, sha) {
        ...
        return this.request(userId, 'PUT', `/repos/${owner}/${repo}/contents/${path}`, {
            message,
            content: encoded,
            branch,
     ...
     * Deletar arquivo
     */
    async deleteFile(userId, owner, repo, path, message, sha, branch) {
        return this.request(userId, 'DELETE', `/repos/${owner}/${repo}/contents/${path}`, { message, sha, branch });
    }
    /**
     * Listar releases
     */
    async listReleases(userId, owner, repo) {
        return this.request(userId, 'GET', `/repos/${owner}/${repo}/releases`);
    }
    /**
     * Criar release
     */
    async createRelease(userId, owner, repo, tagName, name, body, draft = false) {
     ...
     * Fork repositório
     */
    async forkRepo(userId, owner, repo) {
        return this.request(userId, 'POST', `/repos/${owner}/${repo}/forks`);
    }
    /**
     * Pesquisar repositórios
     */
    async searchRepos(userId, query, options) {
        const params = new URLSearchParams();
