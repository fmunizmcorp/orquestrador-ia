...
export const knowledgebaseRouter = router({
  ...
  create: publicProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      const result: any = await db.insert(knowledgeBase).values({
        userId: ctx.userId || 1,
        title: input.title,
  ...
  addSource: publicProcedure
    ...
    .mutation(async ({ input }) => {
      const result: any = await db.insert(knowledgeSources).values({
  ...
  findSimilar: publicProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      const itemsWithScore = allItems
        ...
        .map((i) => {
          const iTags = i.tags as string[] | null;
          if (!iTags || iTags.length === 0) return { item: i, score: 0 };
